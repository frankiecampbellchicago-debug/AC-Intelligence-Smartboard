...
import { IconGlobe, IconGit, IconExternal, IconRefresh, IconPlus, IconTerminal, IconWizard } from '../components/icons'
...
export function TerminalPage(): React.JSX.Element {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Body: terminal (always mounted) + optional live-site preview */}
      <div ref={containerRef} className="relative flex min-h-0 flex-1">
        ...
        >
          ...
          {/* Whiteboard ideas — drag a chip into the terminal to drop a `claude "…"` command. */}
          {wbItems.length > 0 && (
            <div className="flex shrink-0 items-center gap-2 overflow-x-auto border-b border-border bg-surface-2 px-3 py-1.5">
              <span className="shrink-0 text-[10px] font-semibold uppercase tracking-wide text-subtle">
                Ideas →
              </span>
              {wbItems.slice(0, 12).map((it) => (
                <div
                  key={it.id}
                  draggable
                  onDragStart={(e) => {
                  ...
                  title={it.prompt}
                  className="flex shrink-0 cursor-grab items-center gap-1.5 rounded-full border border-border bg-surface px-2.5 py-1 text-xs text-text transition hover:border-accent active:cursor-grabbing"
                >
                  {it.type === 'image' ? (
                    it.dataUrl && (
                      <img src={it.dataUrl} alt="" className="h-4 w-4 rounded object-cover" />
                    )
                  ) : (
                    <IconWizard className="h-3 w-3 text-accent" />
                  )}
                  <span className="max-w-[140px] truncate">{it.prompt}</span>
                </div>
              ))}
            </div>
          )}
                ...
                (() => {
                  ...
                  return (
                    <webview
                      ...
                      style={{
                        width: `${DESKTOP_W}px`,
                        height: size.h > 0 ? `${size.h / scale}px` : '100%',
                        border: '0',
